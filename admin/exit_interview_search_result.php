...
if (isset($_SESSION['UserLogin']) && $_SESSION['Access'] == "Administrator" || $_SESSION['Access'] == "Counselor") {
    ...
    $search1 = $_GET['exitsearch'];
    $collegefilter = $_SESSION['College'];

    $limit = isset($_POST["limit-records"]) ? $_POST["limit-records"] : 10;
    $page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
    $start = ($page - 1) * $limit;
    $sql = "SELECT * FROM exit_interview WHERE name LIKE '%$search1%' AND college='$collegefilter'
    ORDER BY id DESC";
    $appointment = $con->query($sql) or die($con->error);
    $row = $appointment->fetch_assoc();

    $sql01 = "SELECT count(id) AS id FROM exit_interview WHERE college='$collegefilter'";
            ...
            <div class="page-wrapper">
                ...
                <div class="page-breadcrumb">
                    <div class="row align-items-center">
                        <div class="col-5">
                            <h4 class="page-title"><i class="mdi mdi-account-switch"></i> Exit Interview Fill Up Online</h4>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="table-responsive" style="padding: 8px; ">
                                    <!-- exit interview table Pagination -->
                                    <!-- <div class="container" style="padding-bottom: 8px; background-color:aquamarine">
                                        <form action="" method="post" id="form" name="form">
                                            <ul class="pagination pagination" style="margin-left: 20% ;">
                                                <a href="exit_interview_search_result.php?page=<?php echo $previous; ?>" style="padding:8px;">
                                                    &laquo; Previous
                                                </a>
                                                <?php for ($i = 1; $i <= $pages; $i++) { ?>
                                                    <li style="padding:8px; ">
                                                        <a href="exit_interview_search_result.php?page=<?php echo $i; ?>">
                                                            <?php echo $i; ?>
                                                        </a>
                                                    </li>
                                                <?php } ?>
                                                <a href="exit_interview_search_result.php?page=<?php echo $next; ?>" style="padding:8px;">
                                                    Next &raquo;
                                                </a>
                                            </ul>
                                        </form>
                                    </div> -->
                                    <form action="exit_interview_search_result.php" method="get" style="padding:8px;">
                                        <input type="text" required placeholder="Input Name" name="exitsearch">
                                        <button type="submit" class="btn btn-primary btn-block" name="interviewgsearch">Search</button>
                                    </form>
                                        ...
                                        <?php if ($row != null) { ?>
                                            ...
                                            <?php do { ?>
                                                ...
                                                ?>
                                                    <tbody>
                                                        <tr>
                                                            <td style=" white-space: nowrap; ">
                                                                <b>
                                                                    <a href="details_exit_interview.php?ID=<?php echo $row['id']; ?>">Details</a>
                                                                </b>
                                                            </td>
                                                            <td style=" white-space: nowrap; ">
                                                                <?php
                                                    ...
                                                    <?php } else {
                                                    echo '<h4 style="color: red;" >' . '<b>' . 'Exit Interview' . '<b>' . ' (No result!)' . '</h4>';
                                                } ?>
